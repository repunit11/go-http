...
カーネル内のソケットを使う。
finalizer
カーネル空間に影響するようなオブジェクトはライブラリ側が必要に応じてfinalizerを登録していることがある。  
GCで削除対象に選ばれるとfinalizerが実行され、シスコールが呼ばれ後処理する。  
nilを入れただけでは、オブジェクトへの参照が消されるだけで実体は残っている。カーネル空間のソケットなど。
closeをすることで、カーネル空間のソケットを解放することができる。オブジェクトはヒープ領域に残るけどね。

バッファはまとめて取ってくるときに使いたいもの。  
バッファを使うと複数のリクエスト分を取ってこれる可能性がある。  
リクエスト後とにバッファを変えてしまうとリクエストが重複したり欠けていたりするかも。  
メモリ管理の観点からもできるだけバッファは減らしておきたい。  
